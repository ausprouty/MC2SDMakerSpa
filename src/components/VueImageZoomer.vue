...
<template>
    ...
    <div class="vh--outer vh--rel" v-cloak v-click-outside="isZoom" v-show="!showSlot || lazyload">
        <div class="vh--holder vh--rel vh--flex vh--jc" :class="{ 'vh--no-click': !rightClick }"
            @mouseenter="isZoom(true, 'hover')" @mouseleave="isZoom(false, 'hover')" @mousemove="mousePos"
            ref="vue-hover-zs" @click="isZoom(!zoomed, 'click')">
            <picture :class="{ 'vh--none': zoomed }">
                <template v-for="breakpoint in breakpoints" :key="breakpoint.width">
                    <source v-if="breakpoint.regularWebp" :srcset="breakpoint.regularWebp" type="image/webp"
                        :media="'(min-width:' + breakpoint.width + 'px)'" />
                    <source v-if="breakpoint.regular" :srcset="breakpoint.regular"
                        :media="'(min-width:' + breakpoint.width + 'px)'" />
                </template>
            ...
            <picture v-if="zoomed">
                <template v-for="breakpoint in breakpoints" :key="breakpoint.width">
                    <source v-if="breakpoint.zoomWebp" :srcset="breakpoint.zoomWebp" type="image/webp"
                        :media="'(min-width:' + breakpoint.width + 'px)'" />
                    <source v-else-if="breakpoint.regularWebp" :srcset="breakpoint.regularWebp" type="image/webp"
                        :media="'(min-width:' + breakpoint.width + 'px)'" />
                    <source v-if="breakpoint.zoom" :srcset="breakpoint.zoom"
                        :media="'(min-width:' + breakpoint.width + 'px)'" />
                    <source v-else-if="breakpoint.regular" :srcset="breakpoint.regular"
                        :media="'(min-width:' + breakpoint.width + 'px)'" />
                </template>
                <source v-if="options.zoomWebp" :src="options.zoomWebp" type="image/webp" />
                <img :src="options.zoom" class="vh--image vh--abs"
                    :style="{ width: zoomWidth + 'px', transform: 'translate(-' + x + 'px,-' + y + 'px)' }"
                    v-if="!touch" />
                <img :src="options.zoom" class="vh--image vh--abs"
                    :style="'width:' + zoomWidth + 'px;transform:' + touchPosition" v-else />
            </picture>
            <transition name="VueHoverfade">
                <div class="vh--message vh--abs vh--flex vh--jc vh--ai" :class="'vh--message-' + messagePos"
                    v-if="!zoomed && !loading && !clickZoom && !touch && showMessage" v-html="hoverMessage">
                </div>
                <div class="vh--message vh--abs vh--flex vh--jc vh--ai" :class="'vh--message-' + messagePos"
                    v-else-if="!zoomed && !loading && !touch && showMessage" v-html="clickMessage">
                </div>
                <div class="vh--message vh--abs vh--flex vh--jc vh--ai" :class="'vh--message-' + messagePos"
                    v-else-if="!zoomed && !loading && touch && showMessageTouch" v-html="touchMessage">
                </div>
            </transition>
        </div>
        <transition name="VueHoverfade">
            <div class="vh--close vh--abs vh--flex vh--jc vh--ai" :class="'vh--' + closePos"
                v-if="touch && zoomed && loaded" @click.stop="zoomed = false" v-html="'&times;'">
            </div>
            <div class="vh--loading-o vh--abs vh--flex vh--jc vh--ai" v-else-if="loading">
                <div class="vh--loading" v-html="'&#9696;'">
                </div>
            </div>
        </transition>
    </div>
...
export default {
    ...
    props: {
        ...
        hoverMessage: {
            type: String,
            default: '<span class="vh--icon">&#9906;</span> pasar el cursor para agrandar'
        },
        touchMessage: {
            type: String,
            default: '<span class="vh--icon">&#9906;</span> tocar para agrandar'
        },
        clickMessage: {
            type: String,
            default: '<span class="vh--icon">&#9906;</span> Click para agrandar'
        },
        closePos: {
            type: String,
            default: 'top-left'
